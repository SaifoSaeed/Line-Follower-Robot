...
int read_sonar(){
    ...
    usDelay(10);               // 10 µs delay
    
    PORTB = PORTB & 0xFB; // Trigger LOW
    
    // Wait for Echo HIGH (max 30 ms)
    timeout = 0;
...
// Read timer value
t1counts = ((unsigned long)TMR1H << 8) | TMR1L;
distance =  t1counts / 58;
...
void main() {
    ...
    while(1) {
        ...
        // 1. Check LDR State Machine (Keep this logic!)
        if (voltage >= 35) {
            ULTRA_ON &= 0x01; // Clear previous state
            ULTRA_ON |= 0x01; // Set "Dark" bit
        }
        
        else {
            ULTRA_ON |= 0x02; // Set "Light" bit
        }

        // 2. Run Sonar ONLY if Handshake is complete
        if (ULTRA_ON == 0x03){
            read_sonar();
        }

        // 3. Check Distance & Control Buzzer
        // NOTE: Changed 'distance' to 'Distance' (Case Sensitive!)
        if (Distance < 10 && (ULTRA_ON == 0x03)) {
            PORTB |= 0x08;  // Turn on Indicator LED

            // --- NEW BUZZER LOGIC ---
            BUZZING = 1;
            // Simple blocking buzz for demonstration
            PORTD |= BUZZER;
            usDelay(1000);   // Tiny delay to make it audible
            PORTD &= ~BUZZER;
            usDelay(1000);
        }
        else {
            PORTB &= 0xF7;  // Turn off Indicator LED
            BUZZING = 0;
            PORTD &= ~BUZZER; // Ensure Buzzer is OFF
        }
    }
}